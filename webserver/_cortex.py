# got this from https://github.com/Emotiv/cortex-example/blob/master/python/cortex.py
# handels connection and auth with the device
# documentation at: https://emotiv.gitbook.io/cortex-api
...
class Cortex(Dispatcher):
    ...
    def __init__(self, client_id, client_secret, license:str="", debug_mode=False, **kwargs):
    ...
    def handle_stream_data(self, result_dic):
        if result_dic.get('com') != None:
            ...
            eeg_data['eeg'].pop() # remove markers (used for synchronization in communication)
